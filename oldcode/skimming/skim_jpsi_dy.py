...
# for testing:
finloc = '/storage_mnt/storage/user/llambrec/Kshort/files/Run2017E_DoubleMuon_MINIAOD_pv/job0.root'
#finloc = '/storage_mnt/storage/user/llambrec/Kshort/files/RunIIFall17_DYJetsToLL_MINIAODSIM_test/job0/job0.root'
...
jpsivarlist = (['_JPInvMass','_JPRPos','_JPSumPt'])
...
for j in range(nprocess):
	...
	# skim condition 3: pass trigger
	if(int(getattr(tree,'_passTrigger_em'))==0):
		continue

	# skim condition 4: muon invariant mass
	haszmass = False
	for k in range(len(muoninds)):
		for l in range(k+1,len(muoninds)):
			if hasZmass(tree,muoninds[k],muoninds[l]):
				haszmass = True
	#if not haszmass:
	#	continue

	# skim condition 5: exactly one reconstructed JPsi particle with pt in range
	'''if(getattr(tree,'_nJPsis')!=1):
		continue
	if(getattr(tree,'_JPPt')[0]<8):
		continue'''
