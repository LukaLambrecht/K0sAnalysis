    ...
    storeHNLgenInfo(                                                           iConfig.getUntrackedParameter<bool>("storeHNLgenInfo"))
{
    ...
    v0Analyzer            = new V0Analyzer(iConfig, this);
}
...
void multilep::beginJob(){
    ...
    v0Analyzer->beginJob(outputTree);
...
void multilep::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
    ...
    if(!leptonAnalyzer->analyze(iEvent, *(vertices->begin())) and applySkim) return;
    ...
    v0Analyzer->analyze(iEvent, *(vertices->begin()));
